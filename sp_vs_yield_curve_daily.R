...
table(combined.data$SP.Price.Change.Forward.180Day>0,combined.data$T10Y2Y.imputed>0)
chisq.test(table(combined.data$SP.Price.Change.Forward.180Day>0,combined.data$T10Y2Y.imputed>0))
...
# png("./yield_curve_plots_daily/sp_vs_time_yccolor.png")
ggplot(data=combined.data, aes(x=Date, y=Adj.Close, color=Yield.Curve.Status)) +
...
## plot log-scale S&P 500 vs time, highlighting inverted yield curve periods
inversions <- subset(combined.data[!is.na(combined.data$days.since.last.inversion) & 
                                     combined.data$days.since.last.inversion==0,],select=c("Date","Adj.Close"))
head(inversions)
names(inversions)[2] <- "Last.Inversion.Adj.Close"
...
after.inversion <- merge(after.inversion,inversions,by.x="last.inversion",by.y="Date")
