...
ggplot(data=sp.data[!is.na(sp.data$Prev.SP.Direction) & !is.na(sp.data$SP.Price.Change.Forward),]) + 
  ...
  theme_light() + theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = "./momentum_analysis/momentum_charts_1/sp_daily_mo.png")
...
ggplot(data=sp.data[!is.na(sp.data$Prev.SP.Direction) & !is.na(sp.data$SP.Price.Change.Forward) & 
  ...
  scale_y_continuous(limits=c(-0.03,0.03), labels = scales::percent_format(accuracy = 1)) + 
  theme_light() + theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = "./momentum_analysis/momentum_charts_1/sp_daily_mo_last20.png")
...
# View(yearly.mo.dir)
write.csv(yearly.mo.dir, file="./momentum_analysis/momentum_charts_1/daily_momentum_by_year.csv", row.names=FALSE)
...
sp.data.monthly.modern <- sp.data.monthly[!is.na(sp.data.monthly$Prev.SP.Direction) & 
                                            !is.na(sp.data.monthly$SP.Price.Change.Forward) &
                                            sp.data.monthly$Date>=as.Date("1950-01-01"),]
ggplot(data=sp.data.monthly.modern) + 
  geom_boxplot(aes(x=Prev.SP.Direction,y=SP.Price.Change.Forward), outlier.shape = NA, notch=FALSE) + 
  ...
  scale_y_continuous(limits=c(-0.03,0.03), labels = scales::percent_format(accuracy = 1)) + 
  theme_light() + theme(plot.title = element_text(hjust = 0.5))
ggsave(filename="./momentum_analysis/momentum_charts_1/month_vs_month_boxplot_1950_2018.png")
...
chisq.test(sp.data.monthly.modern$Prev.SP.Direction, sp.data.monthly.modern$SP.Price.Change.Forward>0)
# p-value = 0.0008471
chisq.test(sp.data.monthly.recent$Prev.SP.Direction, sp.data.monthly.recent$SP.Price.Change.Forward>0)
# p-value = 0.01508

# 6-month price change blocks
library(lubridate)

sp.data.biannual <- sp.data.monthly[month(sp.data.monthly$Date) %in% c(1,7),]
head(sp.data.biannual$Date)
sp.data.biannual$SP.Price.Change.Forward.6mo <- sp.data.biannual$SP.Price.Next.6Mo / sp.data.biannual$SP.Price - 1
sp.data.biannual$SP.Price.Change.Backward.6mo <- sp.data.biannual$SP.Price / sp.data.biannual$SP.Price.Prev.6Mo - 1
sp.data.biannual$Prev.SP.Direction.6mo[sp.data.biannual$SP.Price.Change.Backward.6mo>0] <- "Up"
sp.data.biannual$Prev.SP.Direction.6mo[sp.data.biannual$SP.Price.Change.Backward.6mo<0] <- "Down"
...
chisq.test(sp.data.biannual.recent$Prev.SP.Direction.6mo, sp.data.biannual.recent$SP.Price.Change.Forward.6mo>0)
